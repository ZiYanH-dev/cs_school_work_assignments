...
# ========== 核心修改：兼容 testcase 和 testcases 目录 ==========
# 优先检查 testcase，如果不存在则检查 testcases
if [ -d "${LAB_ROOT}/testcase" ]; then
  TESTCASE_DIR="${LAB_ROOT}/testcase"
elif [ -d "${LAB_ROOT}/testcases" ]; then
  TESTCASE_DIR="${LAB_ROOT}/testcases"
else
  # 两者都不存在才报错
  echo "❌ ${LAB_ROOT} 下缺少 testcase 或 testcases 子目录"
  exit 1
fi
# ==============================================================
...
echo -e "\n=== 测试总结 ==="
echo "Lab目录: ${LAB_ROOT}"
echo "测试用例目录: ${TESTCASE_DIR}"  # 新增：显示实际使用的测试目录
