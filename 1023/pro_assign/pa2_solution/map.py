    ...
    for row in range(rows):
        ...
        for col in range(cols):
            ...
            if char == ".":
                ...
                game_map.map[row, col] = Tile.ROCK
            elif char == "@":
                game_map.map[row, col] = Tile.BOMB
            elif char.isdigit():
                game_map.map[row, col] = Tile.EMPTY
                game_map.tank_position_map[int(char)] = (col + 0.5, row + 0.5)
            # Since we assume the input is valid, the else clause below is optional.
            # Manually raising exceptions is also out of the syllabus of COMP 1023.
            else:
...
class Map:
    ...
    def map_diff(self):
        ...
        if self.prev_map is None:
            ...
            return np.full(self.map.shape, True)
    ...
    def collides(
        ...
        """
        # TODO: Task 2.1
        # Calculate the rect of the object's hit-box
        rect = (
            x - width / 2,
            y - height / 2,
            x + width / 2,
        ...
        # Check if the object is outside the map
        if not (
            0 < rect[0] < self.cols
            and 0 < rect[1] < self.rows
            and 0 < rect[2] < self.cols
            and 0 < rect[3] < self.rows
        ):
            return None

        # Check if the object collides with any tile
        return {
            (col, row): self.map[row][col]
            for col in range(self.cols)
            for row in range(self.rows)
            if self.map[row][col] != Tile.EMPTY
            and overlap(rect, (col, row, col + 1, row + 1))
        }
        # TODO: Task 2.1 END
        ...
        for i in range(-1, 1 + 1):
            for j in range(-1, 1 + 1):
                if 0 <= col + i < self.cols and 0 <= row + j < self.rows:
                    tile = self.map[row + j][col + i]
                    self.map[row + j][col + i] = Tile.EMPTY
                    self.draw_on(canvas)
                    if tile == Tile.BOMB:
                        self.trigger_bomb(canvas, col + i, row + j, explode)
                        # The function below only works for the GUI application.
                        # It does NOT work for grading and test cases.
                        # canvas.after(
                        #     100, self.trigger_bomb, canvas, col + i, row + j, explode
                        # )
