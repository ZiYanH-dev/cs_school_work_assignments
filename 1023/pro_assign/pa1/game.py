...
def default_random_number_generator():
    ...
    for i in range(2):
        ...
        result.append(candidates[index])
         # Generate random number from 0 to len(candidates) - 1      
    return min(result)
...
def is_valid_move(game_board, current_shape, current_location, current_rotation):
    ...
    #current location is list containing the cordinate of current piece
    #offsets is a list of lists

    ### TASK 3 STARTS HERE ###
    offset=shapes[current_shape][current_rotation]
    cur_x,cur_y=current_location
    for x,y in offset:
        new_row,new_col=cur_x+x,cur_y+y
         #check bounds
         #we have to check bound first before going to the next step
        if new_row<0 or new_row>19:
            return False
        if new_col<0 or new_col>5:
            return False

        #check the cordinates of corresponding new blocks,whether they overlap
        if game_board[new_row][new_col]:
            return False
        
    return True  # this line also needs to change
    ### TASK 3 ENDS HERE ###
...
def move_left(game_board, current_shape, current_location, current_rotation):
    ...
    """
    #current location is the location of current piece
    #index 0 refers to row
    #index 1 refers to col
    ### TASK 4 STARTS HERE ###
    new_current_location=current_location[:]
    new_current_location[1]=new_current_location[1]-1

    if is_valid_move(game_board,current_shape,new_current_location,current_rotation):
        return new_current_location,current_rotation
    
    return current_location,current_rotation # this line also needs to change
    ### TASK 4 ENDS HERE ###
...
def move_down(game_board, current_shape, current_location, current_rotation):
    ...
    ### TASK 6 STARTS HERE ###
    #get the cordinate of new location as a list
    #index 1 refers to column of list current_postion
    #index 0 refers to row...
    new_current_location=current_location[:]
    new_current_location[0]=new_current_location[0]+1
    if is_valid_move(game_board,current_shape,new_current_location,current_rotation):
        return new_current_location,current_rotation
...
def rotate(game_board, current_shape, current_location, current_rotation):
    ...
    if is_valid_move(game_board,current_shape,current_location,new_rotation):
        return current_location,new_rotation
    
    return current_location,current_rotation  # change this line
    ### TASK 7 ENDS HERE ###
...
def gravity_and_merge(game_board):
    ...
    #after merging numbers are next to each other,so easier for checking duplicates
    def has_duplicates(game_board,col):
        for row in range(19):
            if game_board[row][col]==game_board[row+1][col] and game_board[row][col]:
                return True
        return False
    
    def gravity(game_board,col):
        '''
        this function makes sure that all the elements in col
        are at the bottom
        '''
        #given certain column, we start from the bottom
        #skip the elements already at the bottom

        bottom=19
        while bottom>=0 and game_board[bottom][col]:
            bottom-=1
        j=bottom
        for i in range(bottom,-1,-1):
            if game_board[i][col]:
                game_board[j][col],game_board[i][col]=game_board[i][col],game_board[j][col]
                j-=1
                #since we are going up,we need to minus one

    def merge(game_board,col):
        for row in range(19,0,-1):
            #we need to make sure blocks with same value are not 0
            if game_board[row][col]==game_board[row-1][col]!=0:
                new_value=game_board[row][col]*2+1
                game_board[row][col]=new_value
                game_board[row-1][col]=0

    for col in range(6):
        #we need to apply the gravity function first
    ...
    '''for col in range(6):
        for row in range(19,-1,-1):
            ...
            if row >0:
                #if cur is empty(cell euqal to 0)
                #we do 'gravity'
                ...
                #merge
                    ...
                    new_val=game_board[row][col]+game_board[row-1][col]+1
                    game_board[row][col]=new_val
                    game_board[row-1][col]=0
        '''
